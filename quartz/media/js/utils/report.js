...
const deleteTile = async (tileid, card) => {
    ...
    if (tile) {
        const tileUrl = generateArchesURL("arches:tile");
        return $.ajax({
            type: "DELETE",
            url: tileUrl,
...
const removeTileFromResourceTree = (resourceObj, tileid) => {
    ...
    for (const key of resourceKeys) {
        if (resourceObj[key]?.["@tile_id"] == tileid) {
            ...
            for (const item of resourceObj[key]) {
            ...
            resourceObj = removeTileFromResourceTree(resourceObj[key], tileid);
        }
    }
    return resourceObj;
};
...
export default {
    ...
    // getResourceLink returns the correct URL depending on context:
    // - resource editor URL when viewing a resource in the editor
    // - report URL when viewing a published report
            ...
            if (resourceId) {
                ...
                try {
                    if (urlPathname.includes("resource")) {
                        return generateArchesURL("arches:resource_editor", { resourceid: resourceId });
                    } else {
