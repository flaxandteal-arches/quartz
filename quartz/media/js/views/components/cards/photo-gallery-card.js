...
const viewModel = function(params) {
    ...
    this.getUrl = function(tile){
        var url = '';
        var name = '';
        var nodeId = self.fileListNodeId;

        // TODO: This is a fix to show the correct images in the thumbnails
        // this is not a permanent fix, something is overwriting every tile.data with the last
        // uploaded data. Need to find out what is causing this.
        var val;
        try {
            val = JSON.parse(ko.unwrap(tile._tileData))[nodeId];
        } catch (e) {
            val = ko.unwrap(tile.data[nodeId]);
        }

        if (val && val.length == 1) {
            url = self.cleanUrl(val[0].url) || val[0].content;
            name = val[0].name;
        }
