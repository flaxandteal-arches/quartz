// Project override of arches core file-viewer.js.
//
// Only `getDefaultRenderers` differs from upstream: the original required both
// `renderer.type` and `renderer.ext` to exactly equal the file's mime/ext, and
// otherwise fell back to a mime-class wildcard match. That fails for 3D model
// formats whose extensions can't be summarised by a single mime class
// (e.g. .obj is reported as application/x-tgif by some browsers, .stl as
// model/stl, etc.).
//
// This override makes `renderer.ext` accept a comma-separated list and treats
// an extension match as sufficient on its own. The original mime-class
// wildcard branch is kept as a fallback for files whose extension is missing.
//
// Mirrors the backend monkey-patch in quartz.apps._patch_renderer_ext_csv.
// Remove once upstream PR lands.
...
const viewModel = function(params) {
    ...
    this.getDefaultRenderers = function(type, file){
        ...
        var rawFileType = ko.unwrap(type) || '';
        var rawExtension;
        try {
            rawExtension = ko.unwrap(file).split('.').pop();
        } catch (error) {
            rawExtension = file && file.name ? ko.unwrap(file.name).split('.').pop() : undefined;
        }
        var lowerExtension = rawExtension ? rawExtension.toLowerCase() : '';

        this.fileFormatRenderers.forEach(function(renderer){
            var rendererExts = (renderer.ext || '')
                .split(',')
                .map(function(e){ return e.trim().toLowerCase(); })
                .filter(function(e){ return e.length > 0; });
            var excludeExtensions = renderer.exclude
                ? renderer.exclude.split(',').map(function(e){ return e.trim(); })
                : [];

            // Branch 1: extension match alone is sufficient (regardless of mime).
            if (lowerExtension && rendererExts.indexOf(lowerExtension) > -1) {
                defaultRenderers.push(renderer);
                return;
            }

            // Branch 2: mime-class wildcard fallback (unchanged from upstream).
            var splitFileType = rawFileType.split('/');
            var fileTypeClass = splitFileType[0];
            ...
            if (allowableSubType === '*'
                    && fileTypeClass === allowableType
                    && excludeExtensions.indexOf(rawExtension) < 0) {
                defaultRenderers.push(renderer);
            }
        });
