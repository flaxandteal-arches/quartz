        ...
        viewModel: function (params) {
            ...
            if (!params?.compiled){
                ...
                if (Array.isArray(constructionPhasesNode)) {
                    self.production(
                        constructionPhasesNode.map((x) => {
                            ...
                            const assetType = self.getNodeValue(x, { testPaths: [["phase classification", "asset type"], ["phase classification", "monument type"], ["phase classification", "area type"]] });
                            return { assetType, constructionMaterial, constructionTechnique, coveringMaterial, dateConfidence, dateQualifier, displayDate, endDate, interpretationConfidence, method, period, periodLink, phase, phaseDescription, phaseEvidence, startDate, tileid };
                        })
                    );
                }
                ...
                if (Array.isArray(aircraftProductionNode)) {
                    self.production(
                        aircraftProductionNode.map((x) => {
                            ...
                            const tileid = self.getTileId(x);
                            return { aircraftForm, aircraftFunction, aircraftType, constructionMaterial, constructionMethod, constructionTechnique, dateQualifier, displayDate, endDate, interpretationConfidence, method, period, phase, phaseDescription, phaseCertainty, phaseEvidence, placeOfManufacture, startDate, tileid };
                        })
                    );
                }
                ...
                if (Array.isArray(maritimeProductionNode)) {
                    self.production(
                        maritimeProductionNode.map((x) => {
                            ...
                            const tileid = self.getTileId(x);
                            return { builder, constructionMaterial, constructionMethod, constructionTechnique, coveringMaterial, dateQualifier, displayDate, endDate, fixtureType, interpretationConfidence, method, ordnanceType, period, phase, phaseDescription, phaseCertainty, phaseEvidence, propulsionType, riggingType, startDate, tileid, vesselType };
                        })
                    );
                }
                ...
                if (Array.isArray(componentsNode)) {
                    self.components(
                        componentsNode.map((x) => {
                            const constructionPhase = self.getNodeValue(x, { testPaths: [["associated area phase"], ["associated asset construction phase"], ["associated monument construction phase"]] });
                            ...
                            const evidence = self.getNodeValue(x, { testPaths: [["component attribute assignment", "evidence type"], ["component attribute assignment", "component evidence type"]] });
                            const tileid = self.getTileId(x);
                            return { constructionPhase, component, material, technique, evidence, tileid };
                        })
                    );
                }
                ...
                if (self.dataConfig.usePhase) {
                    ...
                    self.usePhases(
                        usePhaseNode.map((x) => {
                            var type = self.getNodeValue(x, { testPaths: [["use phase classification", "functional type"], ["use phase classification", "functional  craft type"]] });
                ...
                if (Array.isArray(organizationFormationNode)) {
                    self.organizationFormation(
                        organizationFormationNode.map((x) => {
                            ...
                            const tileid = self.getTileId(x);
                            return { startDate, endDate, tileid, organizationType, dateQualifier };
                        })
                    );
                }
